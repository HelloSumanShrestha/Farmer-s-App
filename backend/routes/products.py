...
@router.get("/products/search/{name}", response_model=List[Product], tags=["Search"])
async def search_products_by_name(name: str):
    ...
    try:
        today = datetime.now().strftime('%Y-%m-%d')
        search_name = f"%{name.lower()}%"  # Convert search string to lowercase
        await cursor.execute("SELECT * FROM products WHERE LOWER(productName) LIKE %s AND productExpiry >= %s", (search_name, today))
        products = await cursor.fetchall()

        if products is None:  # Check if no products are found
            return []
